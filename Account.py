...
class Account:
    ...
    async def money(self, context, *args):
        ...
        try:
            ...
            await self.client.send_message(
                context.message.channel, context.message.author.mention, embed=em
            )
    ...
    async def levelup(self, context):
        ...
        thumb_url = "https://cdn.discordapp.com/avatars/{0.id}/{0.avatar}.webp?size=1024".format(
            context.message.author
        )
        em.set_thumbnail(url=thumb_url)
        ...
        if confirm.clean_content.upper() == "CONFIRM":
            ...
            await self.client.say(
                context.message.author.mention + " Cancelled level-up."
            )
    ...
    @commands.command(
        name="daily2", aliases=["DAILY2", "bonus", "votebonus"], pass_context=True
    )
    async def daily2(self, context):
        ...
        em = discord.Embed(title="", colour=0x607D4A)
        em.add_field(
            name="Thanks for voting, {}!".format(context.message.author.display_name),
            value=msg,
            inline=True,
        )
        em.set_thumbnail(url=context.message.author.avatar_url)
    ...
    async def toggle_peace(self, context):
        ...
        if user_peace_status == 0 and user_peace_cooldown == 0:
            ...
            em = discord.Embed(title="", colour=0x607D4A)
            em.add_field(
                name=context.message.author.display_name, value=msg, inline=True
            )
            em.set_thumbnail(url=context.message.author.avatar_url)
            await self.client.say(embed=em)
            ...
            if response.clean_content.upper() == "CONFIRM":
                ...
                await self.client.say(
                    context.message.author.mention + " Cancelled peace toggle-on!"
                )
                return
            ...
            if response.clean_content.upper() == "CONFIRM":
                ...
                await self.client.say(
                    context.message.author.mention + " Cancelled peace toggle-off!"
                )
                return
    ...
    @commands.command(
        name="rankings", aliases=["ranks", "leaderboards", "lb"], pass_context=True
    )
